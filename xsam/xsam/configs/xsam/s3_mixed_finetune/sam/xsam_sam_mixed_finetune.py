...
from transformers import AutoModelForCausalLM, AutoTokenizer, SiglipImageProcessor, SiglipProcessor, SiglipVisionModel
...
_DEFAULT_PROFILE_YAML = "profiles/qwen3_1x7b.yaml"
...
image_processor_kind = str(_profile.get("image_processor", "siglip"))
...
s2_pretrained_relpath = _profile.get("s2_pretrained_relpath")
s1_pretrained_pth = None if s1_pretrained_relpath in (None, "null", "") else work_dir + str(s1_pretrained_relpath)
s2_pretrained_pth = None if s2_pretrained_relpath in (None, "null", "") else work_dir + str(s2_pretrained_relpath)
...
if image_processor_kind == "siglip_image":
    _image_processor_type = SiglipImageProcessor.from_pretrained
else:
    _image_processor_type = SiglipProcessor.from_pretrained

image_processor = dict(
    type=_image_processor_type,
