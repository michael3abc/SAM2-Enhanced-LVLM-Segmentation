...
# 1) Segmentor encoder + bridge + m2f decoder from Stage1    (自己的)
# 2) visual/seg projectors from Stage2 (projector-only file) (自己的)
# 3) LLM from extracted lvlm (author)
s1_pretrained_pth = work_dir + "s1_seg_finetune/xsam_sam2_base_768_e3_gpu1_seg_finetune/pytorch_model.bin"
projector_pretrained_pth = init_dir + "extracted_weights/projectors/encoder_llm_projectors.bin"
lvlm_pretrained_pth = init_dir + "extracted_weights/lvlm/xsam_siglip2.bin"
# Stage3 with 4-bit LLM: s2 checkpoint must be projector-only.
...
# Scheduler & Optimizer
batch_size = 1  # per_device (single GPU)
accumulative_counts = 4  # global batch ~= 4
...
train_ratio = dict(
    llava_imgconv=0.1,
    coco_panoptic_genseg=0.1,
    refcoco_refseg=0.1,
    refcoco_plus_refseg=0.1,
    refcocog_refseg=0.1,
    lisa_reaseg=0.1,
    grandf_gcgseg=0.1,
    refcocog_gcgseg=0.1,
...
llava_imgconv_dataset = dict(
    ...
    train_ratio=train_ratio["llava_imgconv"],
)
...
bash run.sh --modes train \
  --config xsam/xsam/configs/xsam/s3_mixed_finetune/xsam_phi3_mini_4k_instruct_siglip2_so400m_p14_384_sam2_base_plus_768_m2f_gpu1_mixed_finetune.py
