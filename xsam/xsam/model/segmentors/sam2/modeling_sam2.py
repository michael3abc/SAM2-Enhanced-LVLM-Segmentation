...
from transformers.utils import ModelOutput, can_return_tuple, is_flash_attn_2_available, is_flash_attn_3_available, logging
...
def is_flash_attention_requested(config):
    """Check whether flash attention is requested in config.

    Args:
        config: Model config object.

    Returns:
        bool: True when flash attention backend is requested.
    """
    return getattr(config, "_attn_implementation", None) in {"flash_attention_2", "flash_attention_3"}


def _get_attention_interface(config, fallback):
    """Resolve attention callable across transformers API versions.

    Args:
        config: Model config object.
        fallback: Fallback attention callable.

    Returns:
        Callable: Attention function to execute.
    """
    ...
    if attn_impl == "flash_attention_2" and not is_flash_attn_2_available():
        logger.warning_once("`flash_attention_2` requested but flash-attn is unavailable. Falling back to SDPA/Eager.")
        if hasattr(ALL_ATTENTION_FUNCTIONS, "valid_keys") and "sdpa" in ALL_ATTENTION_FUNCTIONS.valid_keys():
            return ALL_ATTENTION_FUNCTIONS["sdpa"]
        return fallback

    if attn_impl == "flash_attention_3" and not is_flash_attn_3_available():
        logger.warning_once("`flash_attention_3` requested but flash-attn is unavailable. Falling back to SDPA/Eager.")
        if hasattr(ALL_ATTENTION_FUNCTIONS, "valid_keys") and "sdpa" in ALL_ATTENTION_FUNCTIONS.valid_keys():
            return ALL_ATTENTION_FUNCTIONS["sdpa"]
        return fallback

    if hasattr(ALL_ATTENTION_FUNCTIONS, "get_interface"):
        return ALL_ATTENTION_FUNCTIONS.get_interface(attn_impl, fallback)

    if hasattr(ALL_ATTENTION_FUNCTIONS, "valid_keys") and attn_impl in ALL_ATTENTION_FUNCTIONS.valid_keys():
...
class Sam2MultiScaleAttention(nn.Module):
    ...
    def forward(self, hidden_states: torch.Tensor, **kwargs) -> torch.Tensor:
        ...
        attention_interface: Callable = _get_attention_interface(self.config, eager_attention_forward)
